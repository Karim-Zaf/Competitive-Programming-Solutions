...
const ll N = 1e6 + 30, mod = 1e9 + 7, INF = 1e18 + 10;
...
template <typename tag> struct dynamic_modnum {
     ...
     * Description: Compute $a \% b$ about 5 times faster than usual, where  is constant but not known at compile time.
...
// remove int ll so that it works
...
int32_t main() {
    ...
    ll Test_case = 1;
    cin >> Test_case;
    num::set_mod(mod); //hotha fl main
    num f = 1;
    for (int i = 1; i < N; i++) {
        dp[i] = dp[i - 1];
        dp[i] += i * f * (i - 1) - (i - 1) * dp[i - 1];
        f *= i;
    }
