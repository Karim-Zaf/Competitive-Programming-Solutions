...
const int N = 3e6 + 30, mod = 998244353, INF = 1e18 + 10;
...
void Solve() {
    ...
    for (int i = m; i >= 1; i--) {
        for (int j = i; j <= m; j += i) ans[i] += sv[j];
        ans[i] = add(-1, binpow(2ll, ans[i]));
        for (int j = i + i; j <= m; j += i) ans[i] = add(ans[i], -ans[j]);
    }
