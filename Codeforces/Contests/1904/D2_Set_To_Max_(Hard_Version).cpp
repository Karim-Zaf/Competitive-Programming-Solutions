...
set<int> pos[N] , lezem[N], blaset_b[N];
...
void Solve() {
    ...
    for (int j = 0 ; j<n ; j++){
        if (a[j]>b[j]){cout << "NO\n" ; return;}
        if (a[j]<b[j])lezem[b[j]].insert(j);
        pos[a[j]].insert(j);
        blaset_b[b[j]].insert(j);
    }
    for (int i= 1; i<=n ; i++){

        for (auto p : lezem[i]){
            auto it = pos[i].lower_bound(p);
            if (it != pos[i].end()&&sg.query(p,*it)<=i&&sum.query(p,*it)==0){
                pos[i].insert(p);continue;
            }
            if (it==pos[i].begin()){cout << "NO\n" ; return;}
            it--;
        ...
        for (auto p : blaset_b[i])sum.upd(p,1);
