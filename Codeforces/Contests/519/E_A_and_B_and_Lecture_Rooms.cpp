...
vector<int> graph[N], depth(N), sz(N, 1);
int up[N][30];
...
void Solve() {
    ...
    while (q--) {
        ...
        int lc = Binary_Lifting(u, dist);

        int ans = sz[lc];

        int taht = Binary_Lifting(u, dist - 1);
        int taht2 = Binary_Lifting(v, dist - 1);
        ans -= sz[taht];

        if (lc == Lca(u, v)) ans = n - sz[taht] - sz[taht2];
        if (u == v) ans = n;
        cout << ans << endl;
    }
