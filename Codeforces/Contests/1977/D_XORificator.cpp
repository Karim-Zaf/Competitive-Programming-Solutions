/*    _   __   ___    _   ____    _    _        _____    ___     _____
     | | / /  / _ \  | | |  _ \  | |  | |      |___  |  / _ \   |  ___|
     | |/ /  | |_| | | | | |_| | |  \/  |         / /  | |_| |  | |__
     |   /   |  _  | | | |  _ /  | |\/| |        / /   |  _  |  |  __|
     | |\ \  | | | | | | | | \ \ | |  | |       / /__  | | | |  | |
     |_| \_\ |_| |_| |_| |_| |_| |_|  |_|      |_____| |_| |_|  |_|      */
#include<bits/stdc++.h>

typedef long long ll;
using namespace std;
#ifndef ONLINE_JUDGE

#include "debug.cpp"

#else
#define kar(...)
#endif
#define pb push_back
#define F first
#define S second
#define all(x) x.begin(),x.end()
#define rall(x) x.rbegin(),x.rend()
#define int ll
#define endl '\n'

const int N = 3e5 + 30, mod = 1e9 + 7, INF = 1e18 + 10;

int B1 = 1e9 + 3, B2 = 9974;

int binpow(int a, int k, int res = 1, int Mod = mod) {
    while (k) {
        if (k % 2)res = (res * a) % Mod;
        a = (a * a) % Mod;
        k /= 2;
    }
    return res;
}

int modinv(int a, int Mod = mod) { return binpow(a, Mod - 2); }

int mult(int a, int b, int Mod = mod) {
    b %= Mod;
    return a % Mod * b % Mod;
}

int add(int a, int b, int Mod = mod) { return ((a % Mod + b % Mod) % Mod + Mod) % Mod; }

int divide(int a, int b, int Mod = mod) {
    a %= Mod;
    b = modinv(b) % Mod;
    return (a * b) % Mod;
}

map<array<int, 3>, int> mp;


void Solve() {
    int n, m, mx = 0;
    cin >> n >> m;

    mp.clear() ;
    int t[n][m];

    for (int i = 0; i < n; i++)
        for (int j = 0; j < m; j++) {
            char c;
            cin >> c;
            t[i][j] = c - '0';
        }
    /*
     * COLONNE : 110 --> 110 -- > 000
     */

    for (int j = 0; j < m; j++) {

        array<int, 3> h = {0, 0, 0};
        for (int i = 0; i < n; i++) {
            h[0] = add(h[0], mult(t[i][j] + 1, binpow(B1, i)));
            h[1] = add(h[1], mult(t[i][j] + 1, binpow(B2, i)));
        }


        for (int i = 0; i < n; i++) {
            h[0] = add(h[0], -mult(t[i][j] + 1, binpow(B1, i)));
            h[1] = add(h[1], -mult(t[i][j] + 1, binpow(B2, i)));

            h[0] = add(h[0], mult((t[i][j] ^ 1) + 1, binpow(B1, i)));
            h[1] = add(h[1], mult((t[i][j] ^ 1) + 1, binpow(B2, i)));

            mp[h]++;


            h[0] = add(h[0], -mult((t[i][j] ^ 1) + 1, binpow(B1, i)));
            h[1] = add(h[1], -mult((t[i][j] ^ 1) + 1, binpow(B2, i)));

            h[0] = add(h[0], mult(t[i][j] + 1, binpow(B1, i)));
            h[1] = add(h[1], mult(t[i][j] + 1, binpow(B2, i)));
        }
    }



    for (auto x: mp) {
        mx = max(mx, x.S);
    }
    cout << mx << endl;

    for (int j = 0; j < m; j++) {

        array<int, 3> h = {0, 0, 0};
        for (int i = 0; i < n; i++) {
            h[0] = add(h[0], mult(t[i][j] + 1, binpow(B1, i)));
            h[1] = add(h[1], mult(t[i][j] + 1, binpow(B2, i)));
        }


        for (int i = 0; i < n; i++) {
            h[0] = add(h[0], -mult(t[i][j] + 1, binpow(B1, i)));
            h[1] = add(h[1], -mult(t[i][j] + 1, binpow(B2, i)));

            h[0] = add(h[0], mult((t[i][j] ^ 1) + 1, binpow(B1, i)));
            h[1] = add(h[1], mult((t[i][j] ^ 1) + 1, binpow(B2, i)));

            if (mp[h] == mx) {
                for (int k = 0; k < n; k++)
                    if (k != i)cout << t[k][j];
                    else cout << (t[k][j] ^ 1);
                cout << endl;
                return;
            }

            h[0] = add(h[0], mult(t[i][j] + 1, binpow(B1, i)));
            h[1] = add(h[1], mult(t[i][j] + 1, binpow(B2, i)));

            h[0] = add(h[0], -mult((t[i][j] ^ 1) + 1, binpow(B1, i)));
            h[1] = add(h[1], -mult((t[i][j] ^ 1) + 1, binpow(B2, i)));
        }
    }
}

int32_t main() {
#ifndef ONLINE_JUDGE
    freopen("input.txt", "r", stdin), freopen("output.txt", "w", stdout);
#endif
    ios::sync_with_stdio(0), cin.tie(0);
    int Test_case = 1;
    cin >> Test_case;
    while (Test_case--) Solve();
}