...
void Solve() {
    ...
    cin >> n;
    Seg<int> sg(n + 4);
    vector<int> v(n + 1), pref_mx(n + 3, -INF), suff_mx(n + 4, -INF);

    for (int i = 1; i <= n; i++) cin >> v[i];

    for (int i = 1; i <= n; i++) sg.upd(i, v[i]);
    for (int i = 1; i <= n; i++) pref_mx[i] = max(pref_mx[i - 1], v[i]);
    for (int i = n; i >= 1; i--) suff_mx[i] = max(suff_mx[i + 1], v[i]);


    map<int,int> mp;

    for (int i = 1; i <= n; i++) {
        int one = mp[suff_mx[i]];
        if (i > one + 1) {
            int mn = sg.query(one + 1, i - 1);
            if (mn == suff_mx[i] && mn == pref_mx[one]) {
                cout << "YES\n" << one << " " << i - 1 - one << " " << n - i + 1 << endl;
                return;
            }
        }
        one--;
