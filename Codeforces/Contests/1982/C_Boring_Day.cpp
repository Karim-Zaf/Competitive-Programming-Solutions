...
void Solve() {
    ...
    for (int i = 1; i <= n; i++) {
        ...
        int lef = lower_bound(all(pref), two) - pref.begin();
        int righ = upper_bound(all(pref), one) - pref.begin() - 1;

        int curr = 0;
        if (righ >= 0 && righ < i && lef <= righ) curr = dp.query(0, righ) + 1;
        dp.upd(i, curr);
    }
    cout << dp.query(0, n + 4) << endl;
}
