...
const int N = 3e5 + 30, INF = 1e18 + 10;
...
int sum(int l, int r) {
    int ret = 0;
    if (r / 2 - 1 >= 0)ret = add(-1, binpow(2, r / 2));
    if (l / 2 - 1 >= 0)ret = add(ret, -add(-1, binpow(2, l / 2)));
...
void Solve() {
    ...
    int ans = 0;
    if (l % 2 == 0) {
        ans = add(ans, binpow(2, l / 2 - 1));
        l++;
    }
    if (r % 2) {
        ans = add(ans, binpow(2, (r + 1) / 2 - 1));
        r--;
    }
    ans = add(ans, sum(l, r));
    cout << ans << endl;
